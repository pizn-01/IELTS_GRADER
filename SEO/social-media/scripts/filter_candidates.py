...
from agent_io import THIS_WEEK, blocked_urls, ensure_output_dirs, remember_urls
...
from common import canonical_thread_key, normalize_url, write_csv, ResultRow
...
def _heuristic(row: dict[str, str]) -> FilterResult:
    ...
    if platform in ("reddit", "quora", "twitter"):
        rel += 0.05
    cta_ok = intent in ("tool_ask", "feedback_ask") and platform in CTA_PLATFORMS
    ...
    if platform == "linkedin" and is_linkedin_competitor_promo(title, snippet):
        return FilterResult(
            ...
            benefit=0.1,
            cta_ok=False,
            reason="linkedin_competitor_promo",
            intent=intent,
        )
    return FilterResult(
        url=url,
...
def _llm_batch_score(
    ...
    *,
    ...
    system = (
        ...
        "cta_ok=true ONLY for Quora, Twitter/X, or YouTube when mentioning a free "
        "essay checker is natural (they ask for a tool/score/check). "
        "cta_ok must be false for Reddit and LinkedIn. "
        "Reject LinkedIn competitor AI-tool ads (promo of other checkers). "
    ...
    for i, c in enumerate(batch):
        ...
        if not d:
            out.append(c)
            continue
        cta = bool(d.get("cta_ok", c.cta_ok))
        if (c.platform or "").lower() in NO_PRODUCT_ENGAGE_PLATFORMS:
            cta = False
        out.append(
            FilterResult(
                url=c.url,
...
def filter_discovery_csv(
    ...
    *,
    ...
    for row in raw:
        url = normalize_url(row.get("url") or "")
        key = canonical_thread_key(url) or url.lower().rstrip("/")
        if key and key in blocked:
            rejected.append(
                FilterResult(
                    ...
                    benefit=0,
                    cta_ok=False,
                    reason="seen_or_engaged_or_queued",
    ...
    for c in scored:
        if not c.pass_:
            rejected.append(c)
    # Hard enforce no CTA on Reddit/LinkedIn
    for c in passed:
        if (c.platform or "").lower() in NO_PRODUCT_ENGAGE_PLATFORMS:
            c.cta_ok = False
    ...
    # Remember ALL filtered URLs so they never reappear next week
    remember_urls(
        [{"url": c.url, "platform": c.platform} for c in passed],
        reason="filtered",
    )

    # Side channel: cta_ok map for triage (canonical keys)
    cta_map = {
        (canonical_thread_key(c.url) or normalize_url(c.url).lower().rstrip("/")): c.cta_ok
        for c in passed
    }
    summary = {
        "input_rows": len(raw),
