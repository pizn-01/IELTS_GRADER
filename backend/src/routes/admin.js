...
router.get('/stats', async (req, res) => {
  try {
    ...
    const [
      ...
      { count: totalSubmissionsInPeriod },
      ...
      { count: supportOpenedInPeriod },
      { count: supportResolvedInPeriod },
      ...
      supabaseAdmin.from('submissions').select('*', { count: 'exact', head: true }).gte('created_at', since),
      ...
      supabaseAdmin.from('submissions').select('*', { count: 'exact', head: true }).eq('status', 'grading').gte('created_at', since),
      ...
      supabaseAdmin.from('support_messages').select('*', { count: 'exact', head: true }).gte('created_at', since),
      supabaseAdmin.from('support_messages').select('*', { count: 'exact', head: true }).gte('resolved_at', since).not('resolved_at', 'is', null),
      ...
      supabaseAdmin.from('user_question_assignments').select('*', { count: 'exact', head: true }).gte('assigned_at', since),
    ...
    const totalInPeriod = totalSubmissionsInPeriod || 0;
    const gradedInPeriodCount = gradedInPeriod || 0;
    ...
    return res.json({
      ...
      submissions: {
        ...
        grading_in_period: gradingInPeriod || 0,
        graded_in_period: gradedInPeriodCount,
        failed_in_period: failedInPeriod || 0,
        grading_rate: totalSubmissions ? Math.round(((gradedSubmissions || 0) / totalSubmissions) * 100) : 0,
        grading_rate_in_period: totalInPeriod ? Math.round((gradedInPeriodCount / totalInPeriod) * 100) : 0,
      },
      subscriptions: {
        active: activeSubscriptions || 0,
        canceled: canceledSubscriptions || 0,
        new_in_period: paymentsInPeriod || 0,
      },
      payments: {
      ...
      assignments: {
        ...
        in_period: assignmentsInPeriod || 0,
      },
      ...
      support: {
        ...
        opened_in_period: supportOpenedInPeriod || 0,
        resolved_in_period: supportResolvedInPeriod || 0,
