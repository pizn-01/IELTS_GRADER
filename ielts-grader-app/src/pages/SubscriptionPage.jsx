...
const SubscriptionPage = () => {
  ...
  return (
    <div className="w-full max-w-[1440px] mx-auto px-5 sm:px-8 lg:px-[50px] py-5 lg:py-7 min-h-[calc(100dvh-64px)] flex flex-col text-[#101828]">
      <header className="mb-4 lg:mb-5 shrink-0">
        <h1 className="text-[26px] lg:text-[30px] font-bold text-[#101828]">Your Subscription</h1>
      ...
      {error && (
        <div className="mb-4 text-[13px] font-medium text-[#B42318] bg-[#FEF3F2] border border-[#FECDCA] rounded-xl px-4 py-2.5 shrink-0">
          {error}
        </div>
      )}

      {loading ? (
        <div className="flex-1 bg-white border border-[#D0D5DD] rounded-[20px] p-8 shadow-sm flex items-center justify-center">
          <p className="text-[14px] text-[#98A2B3]">Loading subscription details…</p>
        </div>
      ) : (
        <div className="flex-1 grid grid-cols-1 lg:grid-cols-2 gap-5 lg:gap-7 min-h-0 items-stretch">
          {/* Main card */}
          <section className="bg-white rounded-[20px] border border-[#D0D5DD] shadow-sm flex flex-col min-h-0">
            <div className="px-5 sm:px-8 py-5 border-b border-[#F2F4F7]">
              <div className="flex flex-wrap items-start justify-between gap-3 mb-4">
                <div className="flex items-center gap-2.5 flex-wrap">
                  <h2 className="text-[20px] font-bold text-[#101828]">{planLabel}</h2>
              ...
              <div className="grid grid-cols-3 gap-3 text-center sm:text-left">
                <div className="rounded-lg bg-[#FAFBFC] border border-[#F2F4F7] px-3 py-2.5">
                  <p className="text-[10px] font-semibold uppercase tracking-wide text-[#98A2B3]">Plan</p>
                  <p className="text-[13px] font-semibold text-[#344054] mt-0.5 truncate">{planLabel}</p>
                </div>
                <div className="rounded-lg bg-[#FAFBFC] border border-[#F2F4F7] px-3 py-2.5">
                  <p className="text-[10px] font-semibold uppercase tracking-wide text-[#98A2B3]">Renewal</p>
                  <p className="text-[13px] font-semibold text-[#344054] mt-0.5 truncate">{renewalLabel}</p>
                </div>
                <div className="rounded-lg bg-[#FAFBFC] border border-[#F2F4F7] px-3 py-2.5">
                  <p className="text-[10px] font-semibold uppercase tracking-wide text-[#98A2B3]">Billing</p>
                  <p className="text-[13px] font-semibold text-[#344054] mt-0.5 truncate">{billingLabel}</p>
                </div>
              </div>
            </div>

            <div className="px-5 sm:px-8 py-5 flex-1 flex flex-col justify-center gap-4 min-h-0">
              <div>
                ...
                <div className="h-[7px] bg-[#F2F4F7] rounded-full overflow-hidden">
                  <div
              ...
              <div
                className={`rounded-xl px-3.5 py-2.5 flex gap-2.5 ${
                  statusTone === 'amber'
            ...
            <div className="px-5 sm:px-8 py-4 bg-[#FAFBFC] border-t border-[#F2F4F7] rounded-b-[20px] shrink-0">
              {isSubscribed ? (
                <div className="flex flex-col sm:flex-row flex-wrap gap-2.5 sm:justify-end">
                  <button
                    ...
                    className="w-full sm:w-auto px-5 h-[40px] bg-white border border-[#D0D5DD] rounded-[10px] text-[13px] font-bold text-[#344054] hover:bg-[#F9FAFB] transition-all disabled:opacity-60"
                  >
                  ...
                  {isWeekly && (
                    <button
                      ...
                      disabled={billingLoading || upgradeLoading || cancelLoading}
                      className="w-full sm:w-auto px-5 h-[40px] bg-white border border-[#1A96F3] text-[#1A96F3] rounded-[10px] text-[13px] font-bold hover:bg-[#EFF8FF] transition-all disabled:opacity-60"
                    >
                  ...
                  <button
                    ...
                    disabled={billingLoading || upgradeLoading || cancelLoading}
                    className="w-full sm:w-auto px-5 h-[40px] bg-[#344054] text-white rounded-[10px] text-[13px] font-bold hover:bg-[#1D2939] transition-all shadow-sm disabled:opacity-60"
                  >
                ...
                <div className="flex justify-end">
                  <button
                    ...
                    className="w-full sm:w-auto px-8 h-[40px] bg-[#344054] text-white rounded-[10px] text-[13px] font-bold hover:bg-[#1D2939] transition-all shadow-sm"
                  >
                    View Plans
                  </button>
                </div>
          ...
          {/* Merged context panel */}
          <aside className="bg-white rounded-[20px] border border-[#D0D5DD] shadow-sm flex flex-col min-h-0 h-full">
            <div className="px-5 sm:px-8 py-4 border-b border-[#F2F4F7] shrink-0">
              <h3 className="text-[16px] font-bold text-[#101828]">
                {isSubscribed ? 'Plan details' : 'Account overview'}
              </h3>
            </div>

            <div className="flex-1 px-5 sm:px-8 py-5 flex flex-col gap-5 min-h-0 overflow-y-auto">
              {isSubscribed ? (
                <>
                  ...
                  <div className="rounded-xl bg-[#F9FAFB] border border-[#F2F4F7] px-4 py-3 text-[12px] text-[#667085] leading-relaxed">
                    Use <span className="font-semibold text-[#344054]">Manage Subscription</span> to update your card,
                    view invoices, switch plans, or cancel. Changes apply through our secure billing portal.
                  </div>

                  {isWeekly && (
                    <div className="rounded-xl bg-gradient-to-r from-[#EFF8FF] to-[#F0F9FF] border border-[#B2DDFF] px-4 py-3 flex gap-2">
                ...
                <>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                      ...
                      <p className="text-[13px] text-[#667085] leading-relaxed mb-2">
                      ...
                      <ul className="space-y-1 text-[12px] text-[#667085]">
                        <li className="flex items-center gap-1.5">
                          <span className="w-1 h-1 rounded-full bg-[#12B76A]" />
                          Full band report on first essay
                        </li>
                        <li className="flex items-center gap-1.5">
                          <span className="w-1 h-1 rounded-full bg-[#12B76A]" />
                          Same AI quality as paid plans
                        </li>
                      </ul>
                    </div>
                    <div>
                  ...
                  <div className="rounded-xl bg-[#F9FAFB] border border-[#F2F4F7] px-4 py-3 text-[12px] text-[#667085] leading-relaxed">
                    {isExhausted
                      ? 'Your free credit is used. Tap View Plans to choose Weekly or Monthly and continue grading.'
